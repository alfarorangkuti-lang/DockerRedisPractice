...
import { ChevronRight, Bookmark ,SmartphoneCharging, CircleUserRound, HandCoins, Circle } from "lucide-react"
            ...
            <div className="w-full h-fit grid grid-cols-4 gap-2">
                ...
                {badgeItems.map((item) => {
                    ...
                    <button onClick={() => {router.push(item.route)}} key={item.label} className="w-full rounded-xl bg-midground border border-stroke flex mt-4 py-2 hover:opacity-70 duration-150 cursor-pointer">
                        ...
                        <div className="max-w-20 max-h-24 h-full w-full flex items-center justify-center">
                        ...
                        <div className="flex-1 flex items-center justify-around space-x-8">
                            <div className="flex flex-col items-start gap-1">
                                <span className="tracking-wide text-sm text-left ">{item.label}</span>
                                <span className="tracking-wide text-[10px] text-gray-500 text-left">{item.description}</span>
                            </div>
